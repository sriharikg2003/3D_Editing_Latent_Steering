...
def voxel_grid_to_ply(
    ...
    if len(active_indices[0]) == 0:
        print("Warning: No active voxels found! Saving empty PLY.")
        os.makedirs(os.path.dirname(output_path) or ".", exist_ok=True)
        empty_verts = np.empty(0, dtype=[('x', 'f4'), ('y', 'f4'), ('z', 'f4')])
        from plyfile import PlyData, PlyElement
        PlyData([PlyElement.describe(empty_verts, 'vertex')], text=False).write(output_path)
        return output_path
...
class VoxelProcessor:
    ...
    def filter_edit_regions(self, voxel_src: np.ndarray, voxel_tar: np.ndarray, min_size: int = 100):
        ...
        """
        # Guard: if FlowEdit produced empty voxels, treat as no-edit and preserve source.
        # Without this, XOR(v_src, zeros) = v_src, the whole source object becomes the
        # "edit region", gets flipped to zeros, and every downstream SparseTensor call
        # crashes with "max() on empty tensor".
        if voxel_tar.sum() == 0:
            print("Warning: target voxels are empty — preserving source structure unchanged.")
